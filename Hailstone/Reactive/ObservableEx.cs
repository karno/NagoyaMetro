...
namespace Hailstone.Reactive
{
    public static class ObservableEx
    {
        ...
        {
            return source.Merge(source.While(condition));
        }
        ...
        public static void Cooling<T>(this IObservable<T> source, Action<IEnumerable<T>> next)
        {
            List<T> cache = new List<T>();
            source.Subscribe(i => cache.Add(i),
                () => next(cache));
        }
    }
}